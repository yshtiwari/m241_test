		...
		_stickyMenu: function(){
			...
			if( $stickyMenu.length > 0 ){
				...
				$(window).scroll(function(){
					var $win = $(this);
					var newHeight = 0;
					var count = $('.sticky-menu').length;
					if($('.sticky-menu.active').length > 0)
						newHeight = $('.sticky-menu.active').height();
					var curWinTop = $win.scrollTop() + newHeight;
					if((curWinTop > threshold) && count < 2){
						$clone = $stickyMenu.clone();
						$clone.appendTo($stickyMenu.parent());
						$clone.addClass('active');
					}else{
						if( (curWinTop <= threshold) && count > 1){
							$('.sticky-menu').last().remove();
						}
