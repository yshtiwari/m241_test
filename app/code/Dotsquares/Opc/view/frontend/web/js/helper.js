define(['dotsquaresOpcSelectize', 'jquery', 'underscore', 'uiRegistry'], function (selectize, $, _, registry) {
        ...
        $.fn.decorateSelectCustom = function() {
            ...
            if (select.find('option').length) {
                ...
                newSelect
                    ...
                    .addClass('dotsquares_opc_select_container' + newSelectClass);
                    ...
                    if (option.html().trim().length) {
                        ...
                        if (option.data('ccTypes')) {
                            ...
                            _.each(ccTypes, function (title, code) {
                                ...
                                var ccTypeHtml = $('<div class="dotsquares_opc_cc_wrapper"></div>').attr('data-cc-type', code).attr('title', title);
                                ccPreviewHtml.append(ccTypeHtml);
                                i++;
                            });

                            optionHtml += '<div class="dotsquares_opc_cc_preview">' + ccPreviewHtml.html() + '</div>';
                            if (Object.keys(ccTypes).length > 2) {
                                newOption.addClass('dotsquares_opc_cc_option_long');
                                var ccTypesHtml = $('<div class="dotsquares_opc_cc_types_tooltip_content"></div>');
                                _.each(ccTypes, function (title, code) {
                                    var ccTypeHtml = $('<div class="dotsquares_opc_cc_wrapper"></div>').attr('data-cc-type', code).attr('title', title);
                                    ccTypesHtml.append(ccTypeHtml);
                                });
                                var tooltipContentClass = 'dotsquares_opc_cc_tooltip_content_small';
                                if (Object.keys(ccTypes).length > 4) {
                                    tooltipContentClass = 'dotsquares_opc_cc_tooltip_content_big';
                                }

                                optionHtml += '<div data-icon="&#xf196" class="dotsquares_opc_field_tooltip dotsquares_opc_cc_types_tooltip"><div class="' + tooltipContentClass + ' dotsquares_opc_field_tooltip_content">' + ccTypesHtml.html() + '</div></div>';
                            } else {
                                newOption.addClass('dotsquares_opc_cc_option_short');
                            }
                        }

                        optionHtml += option.html();
                        if (option.attr('data-image')) {
                            newOption.addClass('dotsquares_opc_option_with_image');
                            optionHtml += '<img class="dotsquares_opc_option_image" src="' + option.attr('data-image') + '" />';
                        }
                        ...
                        newOption
                            ...
                            .addClass('dotsquares_opc_select_option' + newOptionClass);
                        newSelect.append(newOption);
                    }
                });

                parent.find('.dotsquares_opc_select_container').scrollbar('destroy');
                parent.find('.dotsquares_opc_select_container').remove();
    ...
    function selectValueInCustomSelect(option, selectNative) {
        ...
        if (!newSelect.hasClass('multiple')) {
            newSelect.find('.dotsquares_opc_select_option').removeClass('selected');
        }
    ...
    var section = '.dotsquares_main_wrapper';

    $(document).on('click',
        section + ' .dotsquares_opc_select_container:not(.disabled) .dotsquares_opc_select_option', function () {
            var option = $(this);
    ...
    $(document).on('change', section + ' select', function (e) {
        ...
        var option = $(section + ' .dotsquares_opc_select_container[data-element-id="' +
            $(this).attr('id') + '"] .dotsquares_opc_select_option[data-value="' + $(this).val() + '"]');
        if (option.length) {
            selectValueInCustomSelect(option, false);
        } else {
            option = $(section + ' .dotsquares_opc_select_container[data-element-id="' +
                $(this).attr('id') + '"] .dotsquares_opc_select_option.selected');
            if (option.length) {
    ...
    $(document).on('focusout', section + ' .dotsquares_opc_select_container:not(.disabled), '
        + section + ' .scroll-wrapper',
        function (event) {
            var newSelect = $(this);
            if (newSelect.hasClass('scroll-wrapper')) {
                newSelect = newSelect.find('.dotsquares_opc_select_container').first();
            }

            if (!newSelect.hasClass('multiple')) {
                setTimeout(function () {
                    if (newSelect.find('.dotsquares_opc_select_option.selected').length) {
                        newSelect.addClass('selected');
                    }
                }, 200);
            }
    ...
    $(document).on('keydown', section + ' .dotsquares_opc_select_container:not(.disabled), '
        ...
        if (e.keyCode === 13) { //enter
            if (newSelect.hasClass('selected')) {
                newSelect.removeClass('selected');
                if (newSelect.find('.dotsquares_opc_select_option.selected').length) {
                    var option = newSelect.find('.dotsquares_opc_select_option.selected').first();
                    newSelect.animate({
                        scrollTop: option.position().top - parseInt(option.attr('data-position-top'))
                    }, 0);
                }
            ...
            newSelect.find('.dotsquares_opc_select_option').each(function () {
                var option = $(this);
